...
void solvePar(int rows, int cols, int iterations, double td, double h, int sleep, double ** matrix, int rank, int lastRank) 
{
    ...
    double * linePrevBuffer = new double[cols];
    double * lineCurrBuffer = new double[cols];
    double * linePrevOutBuffer = new double[cols];
    ...
    for(int k = 0; k < iterations; k++) {
        ...
        if (rank == 0){
            ...
            MPI_Recv(linePrevOutBuffer, cols, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

            // Compute first line using the received line
            for(int j = 1; j < cols - 1; j++) {
                c = matrix[0][j];
                t = linePrevOutBuffer[j];
                b = matrix[1][j];
                l = matrix[0][j - 1];
                r = matrix[0][j + 1];
    ...
    delete[](linePrevOutBuffer);
    delete[](lineNextOutBuffer);
